"""
nm_tc_sr_24h_2.py  —  24-hour EEG simulation using NM_TC_SR  (run 2)

Change from run 1: N2 K-complex rate corrected.
  Run 1 used f_N=4.0 → g_KNa≈1.92, which drove continuous slow oscillations
  (~30-60 K-complexes/min, far above the clinical 1-3/min).
  Run 2 uses f_N=1.5 → g_KNa≈0.90, putting the cortex in an excitable state
  where K-complexes are sporadic (noise/thalamus-triggered), matching N2.
  Spindle generation is unchanged (driven by the TC T-current and Ih circuit).

Outputs (saved next to this script):
  eeg_24h_2.npy      Cortical Vp [mV]  at 100 Hz  (8 640 000 samples)
  vt_24h_2.npy       Thalamic Vt [mV]  at 100 Hz
  stage_24h_2.npy    Stage label (0=W 1=N2 2=N3 3=R) at 100 Hz
  t_24h_2.npy        Time vector [hours] at 100 Hz
  sr_24h_2.npy       SR variables at 1 Hz (86 400×8)
  fig_24h_overview_2.png
  fig_24h_epochs_2.png
"""
...
def _csv_name(stage):
    # Use _2 suffix so run-2 waveforms are cached separately from run-1.
...
def main():
    print('\n=== NM_TC_SR 24-hour EEG simulation  (run 2: N2 K-complex corrected) ===\n')
    ...
    # --- save numpy arrays ---
    print('\nSaving numpy arrays…')
    np.save(os.path.join(HERE, 'eeg_24h_2.npy'),   Vp.astype(np.float32))
    np.save(os.path.join(HERE, 'vt_24h_2.npy'),    Vt.astype(np.float32))
    np.save(os.path.join(HERE, 'stage_24h_2.npy'), stage_arr)
    np.save(os.path.join(HERE, 't_24h_2.npy'),     t_h.astype(np.float32))
    np.save(os.path.join(HERE, 'sr_24h_2.npy'),    nt_24h.astype(np.float32))
    print('  eeg_24h_2.npy   (Vp, 100 Hz, 8 640 000 samples, float32)')
    print('  vt_24h_2.npy    (Vt, 100 Hz)')
    print('  stage_24h_2.npy (stage label, 100 Hz, 0=W 1=N2 2=N3 3=R)')
    print('  t_24h_2.npy     (time in hours, 100 Hz)')
    print('  sr_24h_2.npy    (SR variables, 1 Hz, 86 400×8)')
...
def _plot_overview(t_h, Vp, Vt, stage_arr, sr_scipy, nt_24h):
    ...
    fig.suptitle('24-Hour Sleep Simulation — NM_TC_SR Neural Mass Model  (run 2: N2 K-complex corrected)',
    ...
    plt.tight_layout()
    path = os.path.join(HERE, 'fig_24h_overview_2.png')
...
def _plot_epochs(waves):
    ...
    fig.suptitle('Representative 10-second EEG Epochs per Sleep Stage  (run 2: N2 K-complex corrected)',
                 fontsize=13, fontweight='bold')

    descriptions = {
        'Wake': 'Low amplitude, fast activity (~8 Hz)',
        'N2':   'Sporadic K-complexes (1-3/min) + 12 Hz spindles (Vt)',
    ...
    plt.tight_layout()
    path = os.path.join(HERE, 'fig_24h_epochs_2.png')
    fig.savefig(path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f'  saved {path}')
